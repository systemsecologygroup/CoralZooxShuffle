...
elif tb == 1: # For tb = 1  i.e Tbar0 = 27°C
...
elif tb == 2: # For tb = 2 i.e Tbar0 = 28°C, there is not shuffling under 28 since there are not symbiont with mean thermal tolerance higher than 28°C
    dl_A = 49 # index of 1st duration of stress to plot 
    dl_B = 75 # index of 2nd duration of stress to plot 
    iAB = 20 # in xrange(len(A_B))] # Index of temperature deviations to plot
else:
    sys.exit("There are no simulations with this baseline temperature")
...
# H1 Forcing 2 # for first column of plot
...
Coral1_A = load(fileCoral1_A, allow_pickle = True)
Trait1_A = load(fileTrait1_A, allow_pickle = True)
Symb1_A = load(fileSymb1_A, allow_pickle = True)
fileCoral1_A.close()
fileTrait1_A.close()
fileSymb1_A.close()  

# H1 Forcing 2 # for second column of plot
...
Coral1_B = load(fileCoral1_B, allow_pickle = True)
Trait1_B = load(fileTrait1_B, allow_pickle = True)
Symb1_B = load(fileSymb1_B, allow_pickle = True)
fileCoral1_B.close()
fileTrait1_B.close()
fileSymb1_B.close()  

# H1 Forcing 1, for 3rd column of plot
...
Coral1 = load(fileCoral1, allow_pickle = True)
Trait1 = load(fileTrait1, allow_pickle = True)
Symb1 = load(fileSymb1, allow_pickle = True)
fileCoral1.close()
fileTrait1.close()
fileSymb1.close()   

# H2 Forcing 2 # for 1st column of plot
...
Coral2_A = load(fileCoral2_A, allow_pickle = True)
Trait2_A = load(fileTrait2_A, allow_pickle = True)
Symb2_A = load(fileSymb2_A, allow_pickle = True)
fileCoral2_A.close()
fileTrait2_A.close()
fileSymb2_A.close()  

# H2 Forcing 2 # for 2nd column of plot
...
Coral2_B = load(fileCoral2_B, allow_pickle = True)
Trait2_B = load(fileTrait2_B, allow_pickle = True)
Symb2_B = load(fileSymb2_B, allow_pickle = True)
fileCoral2_B.close()
fileTrait2_B.close()
fileSymb2_B.close()  

# H2 Forcing 1 # for 3rd column of plot
...
Coral2 = load(fileCoral2, allow_pickle = True)
Trait2 = load(fileTrait2, allow_pickle = True)
Symb2 = load(fileSymb2, allow_pickle = True)
fileCoral2.close()
fileTrait2.close()
fileSymb2.close()   
...
for k in xrange(3):
    ...
    if 1+k != 1 and 4+k != 4 and 7+k!=7:
        ...
        ax1.set_ylabel("Coral abundance\n(normalised)", fontsize = fsizeT)
        ax2.set_ylabel("Symbiont abundance\n"+r"($\times 10^6$ cells per cm$^2$)", fontsize = fsizeT)
    ...
    if k != 0:
        ax0.text(1-0.3,ax0_ylab, ax0Lab[k], fontproperties = font, fontsize = fsize)
        ax1.text(1-0.3, 1, ax1Lab[k], fontproperties = font, fontsize = fsize)
        ax2.text(1-0.3, 3.5, ax2Lab[k], fontproperties = font, fontsize = fsize)
    else:
        ax0.text(1-0.95,ax0_ylab, ax0Lab[k], fontproperties = font, fontsize = fsize)
        ax1.text(1-0.95, 1, ax1Lab[k], fontproperties = font, fontsize = fsize)
        ax2.text(1-0.95, 3.5, ax2Lab[k], fontproperties = font, fontsize = fsize)
...
plt.savefig("Figures/Fig%s_%d.pdf"%(tbwhere,Tbar0), bbox_inches = 'tight')

"""
Supplementary figures (might not be included in manuscript)
Countourplot of Average Coral, Symbion, Trait abundances for 3 different time-periods (after 1 year spine-up): 1 year, 1-3 year, 3-10 year

First generate the Countouplot data with the commented code bellow from previous runs 
We did not upload all runs on Github because of space but we uploaded the Countourplot data for plotting the figure here.
"""

# SupFig: ´For forcing 2 only, plot coral and symbiont contourplot density within axis magnitude and duration of stress 
...
for iAB in xrange(len(A_B)):
    for iDur in xrange(len(dur_list)):
        ...
        Coral_Case1 = load(fileCoral1, allow_pickle = True)
        #Trait2_Case1 = load(fileTrait1, allow_pickle = True)
        Symb_Case1 = load(fileSymb1, allow_pickle = True)
        fileCoral1.close()
        #fileTrait1.close()
        fileSymb1.close()
        ...
        Coral_Case2 = load(fileCoral2, allow_pickle = True)
        #Trait_Case2 = load(fileTrait2, allow_pickle = True)
        Symb_Case2 = load(fileSymb2, allow_picke = True)
        fileCoral2.close()
        #fileTrait2.close()
        fileSymb2.close()  
...
"""
Supp Fig Sensitivity to temperature forcing - Symbiont abundance in million cell per cm^2 coral surface 
Countourplot data figure
"""
...
Coral_Case1_Period0 = load(file_Coral_Case1_Period0, allow_pickle = True)
...
Symb1_Case1_Period0 = load(file_Symb1_Case1_Period0, allow_pickle = True)
file_Symb1_Case1_Period0.close()
Symb2_Case1_Period0 = load(file_Symb2_Case1_Period0, allow_pickle = True)
file_Symb2_Case1_Period0.close()
Symb3_Case1_Period0 = load(file_Symb3_Case1_Period0, allow_pickle = True)
file_Symb3_Case1_Period0.close()

file_Coral_Case1_Period1 = open("ContourData/"+str(Tbar0)+"/Coral_Case1_Period1.dat", "r")
Coral_Case1_Period1 = load(file_Coral_Case1_Period1, allow_pickle = True)
...
Symb1_Case1_Period1 = load(file_Symb1_Case1_Period1, allow_pickle = True)
file_Symb1_Case1_Period1.close()
Symb2_Case1_Period1 = load(file_Symb2_Case1_Period1, allow_pickle = True)
file_Symb2_Case1_Period1.close()
Symb3_Case1_Period1 = load(file_Symb3_Case1_Period1, allow_pickle = True)
file_Symb3_Case1_Period1.close()

file_Coral_Case1_Period2 = open("ContourData/"+str(Tbar0)+"/Coral_Case1_Period2.dat", "r")
Coral_Case1_Period2 = load(file_Coral_Case1_Period2, allow_pickle = True)
...
Symb1_Case1_Period2 = load(file_Symb1_Case1_Period2, allow_pickle = True)
file_Symb1_Case1_Period2.close()
Symb2_Case1_Period2 = load(file_Symb2_Case1_Period2, allow_pickle = True)
file_Symb2_Case1_Period2.close()
Symb3_Case1_Period2 = load(file_Symb3_Case1_Period2, allow_pickle = True)
file_Symb3_Case1_Period2.close()

file_Coral_Case2_Period1 = open("ContourData/"+str(Tbar0)+"/Coral_Case2_Period1.dat", "r")
Coral_Case2_Period1 = load(file_Coral_Case2_Period1, allow_pickle = True)
...
Symb1_Case2_Period1 = load(file_Symb1_Case2_Period1, allow_pickle = True)
file_Symb1_Case2_Period1.close()
Symb2_Case2_Period1 = load(file_Symb2_Case2_Period1, allow_pickle = True)
file_Symb2_Case2_Period1.close()
Symb3_Case2_Period1 = load(file_Symb3_Case2_Period1, allow_pickle = True)
file_Symb3_Case2_Period1.close()

file_Coral_Case2_Period2 = open("ContourData/"+str(Tbar0)+"/Coral_Case2_Period2.dat", "r")
Coral_Case2_Period2 = load(file_Coral_Case2_Period2, allow_pickle = True)
...
Symb1_Case2_Period2 = load(file_Symb1_Case2_Period2, allow_pickle = True)
file_Symb1_Case2_Period2.close()
Symb2_Case2_Period2 = load(file_Symb2_Case2_Period2, allow_pickle = True)
file_Symb2_Case2_Period2.close()
Symb3_Case2_Period2 = load(file_Symb3_Case2_Period2, allow_pickle = True)
...
for k in xrange(3):
    ...
    if 1+k == 1:
        ax1.set_title(r"1$^{st}$ year", fontsize = fsize)
    elif 1+k == 2: 
        ax1.set_title(r"1$^{st}-3^{rd}$ year", fontsize = fsize)
    elif 1+k == 3:
        ax1.set_title(r"3$^{th}-10^{th}$ year", fontsize = fsize)
    ...
    if tb == 0:
        ...
        minCorBio = 0.80
        corstep = 2
    #minCorBio = 0.92 # 0.80 for 28°C (tb = 2), 0.84 for 27°C (tb = 1); 0.92 for 26°C (tb = 0) (checked visually)
    levelCoral = linspace(minCorBio, 1, 100)
    ...
    if 1+k == 2: # mean coral biomass within period1
        ...
        if case == 1:
            ...
            cbar.set_label("Coral abundance\n(normalised)", fontsize = fsize, labelpad = lp)
            cbar.set_ticks(arange(minCorBio, 1+0.02, 0.02)[::1]) # ::1 for 26°C and 27°C, ::2 for 28°C
            cbar.ax.tick_params(labelsize=fsize) 
            cbar.set_ticklabels(["%.2f"%temp for temp in arange(minCorBio, 1+0.02, 0.02)[::1]]) # ::1 for 26°C and 27°C, ::2 for 28°C
            ...
            cbar = plt.colorbar(CS, ax = cbaxes1)
            cbar.set_label("Coral abundance\n(normalised)", fontsize = fsize, labelpad = lp)
            cbar.set_ticks(arange(minCorBio, 1+0.02, 0.02)[::corstep]) # ::1 for 26°C and 27°C, ::2 for 28°C
            cbar.ax.tick_params(labelsize=fsize) 
            cbar.set_ticklabels(["%.2f"%temp for temp in arange(minCorBio, 1+0.02, 0.02)[::1]]) # ::1 for 26°C and 27°C, ::2 for 28°C
    ...
    if 4+k == 5: # mean symbiont biomass within period1
        ...
        if case == 1:
            ...
            cbar1.set_label("$S_1$ abundance\n"+r"($\times 10^6$ cells per cm$^2$)", fontsize = fsize, labelpad = lp)
...
plt.savefig("Figures/SuppFig_%d.pdf"%Tbar0, bbox_inches = 'tight')
#splt.show()

"""
# Plot Hypothesis results side by side for a particular Tbar0, duration and magnitude of stres
# Did not update this for a while because the figure were never used in reports
"""
...
for cas in xrange(len(cases)):
    ...
    if 1+k != 1 and 4+k != 4 and 7+k !=7:
        ...
        ax1.set_ylabel("Coral abundance\n(normalised)", fontsize = fsize)
        ax2.set_ylabel("Symbiont abundance\n"+r"($\times 10^6$ cells per cm$^2$)", fontsize = fsize)
        ax0.tick_params(labelleft = True, labelright = False, left = True, right = True, bottom = True, top = True, axis = "y", direction = "in", pad = 2, labelsize = fsize)
        ax1.tick_params(labelleft = True, labelright = False, left = True, right = True, bottom = True, top = True, axis = "y", direction = "in", pad = 2, labelsize = fsize)
        ax2.tick_params(labelleft = True, labelright = False, left = True, right = True, bottom = True, top = True, axis = "y", direction = "in", pad = 2, labelsize = fsize)
            ...
            for i5 in xrange(len(Tl)):
                ...
                for ab in ABindex:
                    ...
                    if Forc[f] == "Forcing1" and dl == 0:  # There is not need to consider duration of stress for forcing 1
                        ...
                        fileTrait.close()
                        fileSymb.close()    
                        # plot coral abundance as normalised biomass
